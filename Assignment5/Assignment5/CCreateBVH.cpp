...
CCreateBVH::CCreateBVH(const std::string& CollisionMeshPath, unsigned int NParticles, size_t LocalWorkSize[3])
    : m_nParticles(NParticles), m_CollisionMeshPath(CollisionMeshPath) {
  m_RotateX = 0;
  m_RotateY = 0;
  m_TranslateZ = -1.5f;

  for (unsigned int i = 0; i < 3; i++) m_LocalWorkSize[i] = LocalWorkSize[i];
  ...
  for (unsigned int i = 0; i < m_nLevels; i++) m_clLevelArrays[i] = NULL;
...
bool CCreateBVH::InitResources(cl_device_id Device, cl_context Context, cl_command_queue CommandQueue) {
  ...
  if (!CreateShaderFromFile("forcefield.vert", m_VSForceField)) return false;

  if (!CreateShaderFromFile("forcefield.frag", m_PSForceField)) return false;
  ...
  if (!LinkGLSLProgram(m_ProgRenderForceField)) return false;
  CHECK_FOR_OGL_ERROR();
  ...
  if (!m_PSystemProgram) return false;
  ...
  glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA32F_ARB, m_volumeRes[0], m_volumeRes[1], m_volumeRes[2], 0, GL_RGBA, GL_FLOAT, pVolume);
  CHECK_FOR_OGL_ERROR();

  cl_image_format volume_format;
  volume_format.image_channel_order = CL_RGBA;
  volume_format.image_channel_data_type = CL_FLOAT;
  m_clVolTex3D = clCreateImage3D(Context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, &volume_format, m_volumeRes[0], m_volumeRes[1], m_volumeRes[2],
                                 (m_volumeRes[0] * sizeof(cl_float4)), (m_volumeRes[0] * m_volumeRes[1] * sizeof(cl_float4)), pVolume, &clError);
...
void CCreateBVH::ReleaseResources() {
  ...
  if (m_clLevelArrays)
    for (unsigned int i = 0; i < m_nLevels; i++) SAFE_RELEASE_MEMOBJECT(m_clLevelArrays[i]);
...
void CCreateBVH::ComputeGPU(cl_context Context, cl_command_queue CommandQueue, size_t LocalWorkSize[3]) {
  ...
  Integrate(Context, CommandQueue, m_LocalWorkSize, 0.003f);
  ...
  // Stream compaction
  Scan(Context, CommandQueue, m_LocalWorkSize);
  Reorganize(Context, CommandQueue, m_LocalWorkSize);
...
void CCreateBVH::OnKeyboard(int Key, int Action) {
  ...
  if (Action == GLFW_PRESS) {
    ...
    if (Key == GLFW_KEY_F) m_ShowForceField = !m_ShowForceField;
  } else {
...
void CCreateBVH::Scan(cl_context, cl_command_queue CommandQueue, size_t LocalWorkSize[3]) {
  cl_int clErr;
  ...
  while (offset < N) {
    ...
    if (offset == 1)
      clErr = clSetKernelArg(m_ScanNaiveKernel, 0, sizeof(cl_mem), (void*)&m_clAlive);
    else
...
void CCreateBVH::Integrate(cl_context, cl_command_queue CommandQueue, size_t LocalWorkSize[3], float dT) {
  cl_int clErr;
  size_t globalWorkSize[1];

  cl_float4* pVerts = new cl_float4[m_nTriangles * 3];

  V_RETURN_CL(clEnqueueReadBuffer(CommandQueue, m_clTriangleSoup, CL_TRUE, 0, m_nTriangles * 3 * sizeof(cl_float4), pVerts, 0, NULL, NULL),
...
void CCreateBVH::Reorganize(cl_context, cl_command_queue CommandQueue, size_t LocalWorkSize[3]) {
  //********************************************************
  // GPU Computing only!
