...
CAssignment5* CAssignment5::s_pSingletonInstance = NULL;

CAssignment5* CAssignment5::GetSingleton() {
  if (!s_pSingletonInstance) s_pSingletonInstance = new CAssignment5();
...
bool CAssignment5::EnterMainLoop(int argc, char** argv) {
...
bool CAssignment5::DoCompute() {
  if (m_pCurrentTask) m_pCurrentTask->ComputeGPU(m_CLContext, m_CLCommandQueue, m_LocalWorkSize);
...
bool CAssignment5::InitCLContext() {
...
// Define OS-specific context properties and create the OpenCL context
  ...
  cl_context_properties props[] = {CL_GL_CONTEXT_KHR, (cl_context_properties)glXGetCurrentContext(), CL_GLX_DISPLAY_KHR,
                                   (cl_context_properties)glXGetCurrentDisplay(), CL_CONTEXT_PLATFORM, (cl_context_properties)m_CLPlatform, 0};
#else  // Win32
  cl_context_properties props[] = {CL_GL_CONTEXT_KHR, (cl_context_properties)wglGetCurrentContext(), CL_WGL_HDC_KHR, (cl_context_properties)wglGetCurrentDC(),
                                   CL_CONTEXT_PLATFORM, (cl_context_properties)m_CLPlatform, 0};
#endif
#endif
...
bool CAssignment5::InitGL(int, char**) {
...
void CAssignment5::CleanupGL() {
  glfwDestroyWindow(m_Window);
  glfwTerminate();
}

void CAssignment5::Render() {
  if (m_pCurrentTask) m_pCurrentTask->Render();

  glfwSwapBuffers(m_Window);
}

void CAssignment5::OnIdle() {
  if (m_PrevTime < 0) {
    m_FrameTimer.Start();
    m_PrevTime = 0.0;
  } else {
    ...
    if (m_pCurrentTask) m_pCurrentTask->OnIdle(time, elapsedTime);

    DoCompute();
  }
}

void CAssignment5::OnWindowResizedCallback(GLFWwindow* pWindow, int Width, int Height) {
  GetSingleton()->OnWindowResized(pWindow, Width, Height);
}

void CAssignment5::OnWindowResized(GLFWwindow*, int Width, int Height) {
  if (m_pCurrentTask) m_pCurrentTask->OnWindowResized(Width, Height);
}

void CAssignment5::OnKeyboardCallback(GLFWwindow* pWindow, int Key, int ScanCode, int Action, int Mods) {
  GetSingleton()->OnKeyboard(pWindow, Key, ScanCode, Action, Mods);
}

void CAssignment5::OnKeyboard(GLFWwindow*, int Key, int, int Action, int) {
  if (Key == GLFW_KEY_ESCAPE && Action == GLFW_PRESS) glfwSetWindowShouldClose(m_Window, GL_TRUE);

  if (m_pCurrentTask) m_pCurrentTask->OnKeyboard(Key, Action);
}

void CAssignment5::OnMouseCallback(GLFWwindow* pWindow, int Button, int State, int Mods) {
  GetSingleton()->OnMouse(pWindow, Button, State, Mods);
}

void CAssignment5::OnMouse(GLFWwindow*, int Button, int State, int) {
  if (m_pCurrentTask) m_pCurrentTask->OnMouse(Button, State);
}

void CAssignment5::OnMouseMoveCallback(GLFWwindow* pWindow, double X, double Y) {
  GetSingleton()->OnMouseMove(pWindow, X, Y);
}

void CAssignment5::OnMouseMove(GLFWwindow*, double X, double Y) {
  if (m_pCurrentTask) m_pCurrentTask->OnMouseMove((int)X, (int)Y);
}
